---
title: "Geospatial Techniques for Social Scientists"
...
.pull-left[
There are a lot of models you could estimate to *explain* spatial autocorrelation. And there's a vast body of literature on what's the best choice for which application. I'd explicitly like to recommend you the work of [Tobias Rüttenauer](https://ruettenauer.github.io/publication/) (guy on the right) for us social scientists. [Here](https://ruettenauer.github.io/SICSS-Spatial/index.html) are some really nice workshop materials.
]

.pull-right[
.center[
```{r tobi, echo = F}
woRkshoptools::include_picture("at_home.png")
```
]
]
...
Unfortunately, in case of a Spatial Lag X Model the spatial parameter $\rho$ only tells us that the effect is (statistically) significant -- or not. 
...
election_results_missing$afd_share[missing_index] <- NA
election_results_missing$afd_share[missing_index] <-
  mean(election_results_missing$afd_share, na.rm = TRUE)
```
]

.pull-right[
.center[
```{r mean-impute-2, echo = FALSE}
# ~10% missing values
missing_index <- 
  sample(
    1:nrow(election_results), 
